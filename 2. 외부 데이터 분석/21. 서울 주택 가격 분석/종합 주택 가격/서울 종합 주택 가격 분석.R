##### 서울 집값(종합) 시계열 분석 및 예측 #####

### Read csv file - 서울 집값(종합)
# seoul_total.csv 파일을 읽어오면 된다.
seoul.total <- read.csv(file.choose(), stringsAsFactors = F)
str(seoul.total)

### 변수명 "연도" -> "year"로 변경
names(seoul.total)[2] <- "year"

### 1986년 ~ 2017년으로 한정 & 1,2열 제외
seoul.total <- seoul.total[1:32, -(1:2)]
...
for(i in 1:32) {
  for(j in 1:12) {
    seoul.t <- c(seoul.t, seoul.total[i, j])
  }
}

### 시계열로 인식
seoul.ts <- ts(seoul.t, frequency = 12, start = c(1986, 1))
...
### 1. 시계열 그림 그리기
plot(seoul.ts)
abline(h = mean(seoul.ts), col = "blue")
...
# p-value = 0.6425이므로 H0를 기각할 수 없다.
...
### 3. 차분
d.seoul <- diff(seoul.ts)
plot(d.seoul)
abline(h = mean(d.seoul), col = "blue")

## 1차 차분을 통한 결론 ##
# 1차 차분을 한 시계열 그림은 분산이 일정해보인다.
# 그대로 분석을 진행한다.
...
# PACF에서 lag = 16에서 파란선 위로 선이 약간 나온 형태를 보이고 있으나, 0으로 봐도 무방해보인다.
# 따라서, 모형은 AR(13)임을 알 수 있다.

### 5. 모형 식별
ar(d.seoul, order.max = 20, method = "yule-walker")$aic

## 결과 해석 ##
# lag = 13에서 통계량 AIC 값이 0이 된 것을 볼 수 있다.
# 따라서, AR(13)로 모형을 잘 적합하였다는 것이 확인되었다.
# 이제, 모수를 추정해주자.

### 6. 모수 추정 및 모형 적합
fit <- arima(d.seoul, order = c(13, 0, 0))
fit

## 결과 해석 ##
# 모형 : Z(t) - 0.1576 = 0.7922(Z(t-1) - 0.1576) - 0.0141(Z(t-2) - 0.1576) + ... - 0.1292(Z(t-13) - 0.1576)
...
# (1) 차분을 안 한 데이터에서의 예측
forecast(arima(seoul.ts, order = c(13, 0, 0)), h = 48)
plot(forecast(arima(seoul.ts, order = c(13, 0, 0)), h = 48))
